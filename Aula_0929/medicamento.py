...
class Medicamento:
  ...
  def to_json(self):
    return {
      '__id': self.__id,
      '__descricao': self.__descricao,
      '__valor': self.__valor,
      '__vencimento': self.__vencimento.strftime('%d/%m/%Y')
    }
    ...
    try:
      ...
      with open('medicamentos.json', 'r') as arquivo:
        ...
        for medicamento in a:
          m = Medicamento(
            medicamento['__id'], medicamento['__descricao'],
            medicamento['__valor'],
            datetime.strptime(medicamento['__vencimento'], '%d/%m/%Y'))
...
class UI:
  ...
  def menu(cls):
    print(
      '0 - sair | 1 - inserir | 2 - listar | 3 - atualizar | 4 - excluir | 5 - vencidos'
    )
    return int(input())
  ...
  def inserir(cls):
    ...
    vencimento = datetime.strptime(input('Vencimento no formato dd/mm/aaaa: '),
                                   '%d/%m/%Y')
  ...
  def atualizar(cls):
    ...
    vencimento = datetime.strptime(
      input('Novo vencimento no formato dd/mm/aaaa: '), '%d/%m/%Y')
  ...
  @classmethod
  def vencidos(cls):
    for medicamento in NMedicamento.vencidos():
      print(medicamento)
