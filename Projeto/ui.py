...
class UI:
  ...
  def Menu(cls):
    ...
    print("\nAgenda: ")
    print("9-Inserir, 10-Listar, 11-Atualizar, 12-Excluir, 13 - Abrir Agenda do Dia")
    print("\nOutras Opções: ")
    print("99 - Sair")
    return int(input())
  ...
  def Agenda_Inserir(cls):
    ...
    agenda = Agenda(0, datetime.datetime.strptime(data, "%d/%m/%Y %H:%M"), confirmado, id_cliente, id_servico)
  ...
  def Agenda_Listar(cls):
    for agenda in NAgenda.listar():
      ...
      nome_cliente = ""
      desc_servico = ""
      if id_cliente != 0: nome_cliente = NCliente.listar_id(id_cliente).get_nome()
      if id_servico != 0: desc_servico = NServico.listar_id(id_servico).get_descricao()
      print(agenda, end="")
  ...
  def Agenda_Atualizar(cls):
    ...
    UI.Servico_Listar()
    id_servico = int(input("Id do serviço: "))
    agenda = Agenda(id, datetime.datetime.strptime(data, "%d/%m/%Y %H:%M"), confirmado, id_cliente, id_servico)
